...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    char *base = basecfg(cfgfile);//读取网络配置文件，此函数位于utils.c,返回cfg文件不带后缀的名字
    printf("%s\n", base);
    float avg_loss = -1;
...
#endif
        nets[i] = load_network(cfgfile, weightfile, clear);//解析网络结构，装载权重文件
    ...
    float jitter = l.jitter; //抖动产生额外数据 
    ...
    while(get_current_batch(net) < net->max_batches){
        if(l.random && count++%10 == 0){
            printf("Resizing\n");
			//根据训练图片的大小调节下面10,10,32参数，该处图片防缩范围在100-280之间
            int dim = (rand() % 10 + 10) * 32;
			//最后200次迭代,图片大小为280*608
...
{
    list *options = read_data_cfg(datacfg);//用自定义链表options存储训练集基本信息，函数位于option_list.c,读取cfg/voc.data
	/*voc.data
	classes= 20
    ...
    char **names = get_labels(name_list);//加载位于data/labels下的字符图片，用于显示矩形框名称

    image **alphabet = load_alphabet();//提取了许多预设的字符？data/labels/%d_%d.png
	//用netweork.h中自定义的network结构体存储模型文件,读取权重和配置文件,函数位于network.c,其中又包含了parse_network_cfg(cfgfile)在parser.c，load_weights(net, weightfile)读取yolo.cfg,yolo.weights
	network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    srand(2222222);
    double time;
    ...
    while(1){
        ...
        image sized = letterbox_image(im, net->w, net->h); // 输入图片大小经过resize和填充至网络格式大小
...
void run_detector(int argc, char **argv)//darknet.c中如果命令如果第二个参数是detector，则调用这个
{	//寻找是否有参数prefix
    char *prefix = find_char_arg(argc, argv, "-prefix", 0);//prefix默认参数是0，argv为二维数组，存储了参数字符串
	//寻找是否有参数thresh参数，thresh为输出的阈值
    float thresh = find_float_arg(argc, argv, "-thresh", .24); //thresh默认参数为0.24
	//寻找是否有参数hier_thresh,默认为0.5
    float hier_thresh = find_float_arg(argc, argv, "-hier", .5);
	//寻找是否有参数cam_index,默认为0
    int cam_index = find_int_arg(argc, argv, "-c", 0);
	//寻找是否有参数frame_skip,默认为0
    ...
    int fullscreen = find_arg(argc, argv, "-fullscreen");//检查是否全屏参数
    int width = find_int_arg(argc, argv, "-w", 0);//检查是否设置网络width参数
    int height = find_int_arg(argc, argv, "-h", 0);//检查是否设置网络height参数
    int fps = find_int_arg(argc, argv, "-fps", 0);//检查是否设置frame per second
	...
	// 根据第三个参数的内容（主要是test/train/valid/demo），调用不同的函数，并传入之前从输入中解析到的各种参数
