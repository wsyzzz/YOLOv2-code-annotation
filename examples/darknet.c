...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "average")){
        average(argc, argv);
    } else if (0 == strcmp(argv[1], "yolo")){//应用yolov1
        run_yolo(argc, argv);
    ...
    } else if (0 == strcmp(argv[1], "detect")){  //用于示例演示的选项，相当于detector指定了test模式和训练信息，
        float thresh = find_float_arg(argc, argv, "-thresh", .24);
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outfile = find_char_arg(argc, argv, "-out", 0);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        test_detector("cfg/coco.data", argv[2], argv[3], filename, thresh, .5, outfile, fullscreen);//直接进入test_detector，只考虑了阈值，待检测文件，输出文件，全屏的设置
