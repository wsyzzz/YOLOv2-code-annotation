...
network *parse_network_cfg(char *filename)
{
	...
	node *n = sections->front;   //判断是否为空，并令n为全局[net]设置
    if(!n) error("Config file has no sections");
    
	network *net = make_network(sections->size - 1); //为初始化网络分配空间
    net->gpu_index = gpu_index;
    

    section *s = (section *)n->val;
    list *options = s->options;     //把sections的全局设置取出放到options中
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, net);  //把options中的设置转换到net结构体中

	size_params params;   //size_params指网络尺寸的配置参数
    params.h = net->h;
    params.w = net->w;
    params.c = net->c;
	...
	while(n){
        ...
        s = (section *)n->val;  //这里的n是cfgfile中第二部分开始，s之前也free过了
        options = s->options;
        layer l = {0};
...
list *read_cfg(char *filename)  //输入cfgfile，输出option
{
